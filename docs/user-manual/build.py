...
def build_docx(chapters, out_dir):
    doc = S.new_document(
        header_title="Activation Planner — User Manual",
        header_sub="Pre-operation planning  ·  POTA / SOTA / Field Day / EMCOMM",
        footer_left="Activation Planner  ·  User Manual  ·  KE4CON",
    )
    S.cover(
        doc,
        ...
        version="v1.0",
        tagline="Plan a great operating session — the right bands from real VOACAP propagation, the right antenna from your own gear, and a packing list that's ready to go.",
    ...
    out = os.path.join(out_dir, "Activation_Planner_User_Manual.docx")
    doc.save(out)
    return out
...
def main():
    ...
    md_path = os.path.join(HERE, "Activation_Planner_User_Manual.md")
