...
| Hook | This repo (Activation Planner) |
|---|---|
| **Languages / stack** | C# .NET 10 + Avalonia 12 (Windows/macOS/Linux) — layered ProcessEngine → PropagationModel → Services → UI |
| **Build command** | `dotnet build` |
| **Test command** | `dotnet test` |
| **Coverage command** | `dotnet test --collect:"XPlat Code Coverage"` |
| **Static-analysis / lint** | .NET analyzers, nullable enabled (C# 12) |
| **Run / launch the app** | `dotnet run --project ActivationPlanner.UI` |
| **Input-parsing surfaces** (fuzz targets) | the **VOACAP output parser** (fixed-column, bug-prone), the **NEC2++ geometry/output parser**, POTA HTTP responses, gear/checklist import; also round-trip the VOACAP input-deck *writer* |
| **Reference oracle** | real **VOACAP** and **NEC2++** tool output — diff the parsers against the actual tools; plus the dipole empirical-comparison harness |
| **Concurrency hot spots** | async **`Process.Start` shell-outs** to VOACAP/NEC2++ (ProcessEngine), POTA HTTP, external NMEA GPS (serial), background propagation sampling (~15–30 min) — never block the UI thread |
| **Security-sensitive surfaces** | POTA HTTP (unauthenticated), the process shell-outs (keep GPLv2 NEC2++ strictly out-of-process), `System.Text.Json` persistence |
| **Canonical spec / docs to check against** | `Activation_Planner_Decisions_Log.md`, `CLAUDE.md` (Option A/B trigger thresholds, VOACAP card-format constants) |
| **Known-latent list** | Decisions Log; the **provisional dipole ~0.05λ threshold** — validate empirically against real VOACAP output before treating it as final |
